...
import MultiUserInfoWithInvoiceCol from "../../components/table/MultiUserInfoWithInvoiceCol";
...
const ClassroomList = () => {
    ...
    const fetchData = () => {
        ...
        const params = {
            ...
            setClassroom: setClassrooms,
            config: config,
            setConfig: setConfig,
            setFetching: setFetching
        }
        app.getClassroomListAction(params).then()
    }
    const startFilter = () => {
        fetchData()
    }
    ...
    const confirmDelete = (id) => {
        app.deleteClassroom({id: id, setFetching: setFetching, fetchData: fetchData}).then()
    }
    return (
        <MainLayout>
            <div className={"p-3"}>
                ...
                <div className={"max-h-[65vh] overflow-auto"}>
                    <table className={"table border shadow-lg min-w-max w-full"}
                        ...
                        <thead>
                        <tr>
                            <HeadTable label={'Tên lớp'} style={{zIndex: "30", left: 0}}/>
                            {/*<HeadTable label={'Họ và tên'} style={{position: "sticky", zIndex: "30", left: "10rem"}}/>*/}
                            <HeadTable label={'Chương trình học'}/>
                            <HeadTable label={'Học phí'}/>
                            <HeadTable label={'Thời lượng'}/>
                            <HeadTable label={'Số buổi'}/>
                            <HeadTable label={'Giáo viên'}/>
                            <HeadTable label={'Trợ giảng'}/>
                            <HeadTable label={'Học sinh'}/>
                            <HeadTable label={'Trạng thái lớp'}/>
                        ...
                        {!fetching ?
                            ...
                            {classrooms.map((classroom, key) =>
                                <tr key={key}>
                                    ...
                                    />
                                    <TextCol
                                        value={classroom.program || ''}
                                    />
                                    <TextCol
                                        value={classroom.pricing + " đ" || ''}
                                    />
                                    <TextCol
                                        value={classroom.duration + " phút" || ''}
                                    />
                                    <TextCol
                                        value={classroom.session || ''}
                                    />
                                    <MultiUserInfoCol
                                        entity={"teacher"}
                                    ...
                                    <MultiUserInfoCol
                                        entity={"teacher"}
                                        users={classroom.supporters || []}
                                    />
                                    <MultiUserInfoWithInvoiceCol
                                        entity={"student"}
